...
- Grill 先定位主考点、题干判别词和真实错因；有可访问真题库时检索同考点真题，未找到时明确标注后续为变式题。
...
- 门控卡只提供“开始 Grill / 只看解析 / 今天到这”，不要求填写理由，也不提前公布答案。
- 练习卡只给可点选的内容选项；用户点选前不标记正确项。
- 每题先围绕一个真实错因出题。正确项对应当前考点，干扰项来自用户误选、相邻概念或题型陷阱。
- 若可访问的真题库中没有同考点依据，Skill 会说明这一缺口，并把后续内容标为“变式题”。
