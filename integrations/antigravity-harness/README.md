...
3. 在完全空闲的 model_stop 阶段，发现完整讲题或 Grill 收尾缺少 ask_question 时，让 Agent 继续补发卡片。
...
同一题的后续 Grill 回复会沿用当前已确认的题目归属，避免只因 G1/G2 反馈没有五段式或 `### Grill` 标题就失去 Hook 保护。达到掌握证据可提前结束追问，但仍需归档确认卡；明确结束、拒绝归档或已作出归档选择时不重复发卡。
